...
export function Navbar() {
  const { mode, toggleMode, domain, setDomain, language, setLanguage } =
    useExplanation();
  const pathname = usePathname();
  const router = useRouter();

  const handleDomainChange = (newDomain: "DSA" | "DBA") => {
    setDomain(newDomain);
    if (pathname !== "/") {
      router.push("/");
    }
      ...
      <div className="container mx-auto px-4 flex h-16 items-center justify-between relative">
        ...
        <div className="flex items-center gap-2 sm:gap-6">
          ...
          <div className="flex items-center bg-secondary/30 p-1 rounded-lg border border-border/50 gap-1">
            ...
            <Link href="/patterns">
               <button className={`px-2 sm:px-3 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all duration-200 ${
                  pathname === "/patterns"
                    ? "bg-background shadow-sm text-foreground" 
                    : "text-muted-foreground hover:text-foreground hover:bg-background/50"
               }`}>
                  {language === "tr" ? "Pattern" : "Patterns"}
               </button>
            </Link>
            ...
            <button
              onClick={() => handleDomainChange("DBA")}
              className={`px-2 sm:px-3 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all duration-200 ${
                domain === "DBA" && pathname !== "/patterns"
              ...
              <div className="flex justify-between space-x-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {language === "tr"
                      ? "Mülakatlar için seçilmiş en iyi listeler."
                      : "Curated lists for interview prep."}
                  </p>
                  <div className="flex flex-col gap-2 pt-4">
                    <Link
                      href="https://leetcode.com/studyplan/leetcode-75/"
                      target="_blank"
                      className="flex items-center gap-2 text-sm hover:underline hover:text-primary"
                    >
                    ...
                    <Link
                      ...
                      className="flex items-center gap-2 text-sm hover:underline hover:text-primary"
                    >
                      <BookMarked className="w-4 h-4 text-emerald-500" /> Top
                      Interview 150
                    </Link>
                    <Link
                      ...
                      className="flex items-center gap-2 text-sm hover:underline hover:text-primary"
                    >
                      <BookMarked className="w-4 h-4 text-blue-500" /> Top 100
                      Liked
                    </Link>
                    <Link
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 border rounded-full px-2 py-1 sm:px-3 sm:py-1.5 bg-secondary/50">
            ...
            />
            <span
              className={`text-[10px] sm:text-xs font-medium transition-colors flex items-center gap-0.5 ${
                mode === "eli5"
          ...
          <Link href="https://buymeacoffee.com/lxppyter" target="_blank">
            <Button
              variant="ghost"
              size="icon"
              className="hidden lg:flex w-9 h-9 text-yellow-500 hover:text-yellow-600"
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                  ...
                  <DropdownMenuItem asChild>
                    <Link
                      href="https://buymeacoffee.com/lxppyter"
                      target="_blank"
                      className="flex items-center gap-2 w-full cursor-pointer text-yellow-600 dark:text-yellow-500"
                    >
